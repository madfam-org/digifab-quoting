# Digifab Quoting Agent Operating Guide
...
- **PMF measurement**: per RFC 0013 (`internal-devops/rfcs/0013-pmf-via-coforma-and-tulana.md`), NPS + Sean Ellis + retention via `@madfam/pmf-widget` → Tulana `/v1/pmf/*` endpoints. Composite PMF Score informs `recommended_action` (enable_paywall / measure_more / sunset / keep_measuring).
  - **Integration status (2026-04-26):** Wired into `apps/web` via `src/components/pmf/PmfWidgetMount.tsx`, mounted in the root `app/layout.tsx` inside `<Providers>` (renders nowhere visible until activated). Gates: feature flag `NEXT_PUBLIC_PMF_WIDGET_ENABLED` (default `false`) + NextAuth `session.status === 'authenticated'` + path-prefix exclusion list (`/login`, `/auth`, `/upload`, `/quote`, `/checkout`, `/for`, `/pricing`, `/features`, `/try`) plus exact-match `/`. `productSlug=cotiza`, `apiUrl=$NEXT_PUBLIC_TULANA_API_URL` (default `https://api.tulana.madfam.io`), triggers: NPS afterSession=4 / dismissCooldown=30d, Sean Ellis afterSession=2 / dismissCooldown=45d, smile after 1 `quote_accepted` action. Activation is operator-gated: requires (1) `NPM_MADFAM_TOKEN` rotation so `@madfam/pmf-widget@^0.1.0` can publish + install, (2) deletion of `apps/web/src/types/madfam-pmf-widget.d.ts` once the published `.d.ts` ships, (3) flipping `NEXT_PUBLIC_PMF_WIDGET_ENABLED=true` in the deployed env. The dynamic import is fail-closed — a missing module never breaks the page.
